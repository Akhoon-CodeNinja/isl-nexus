...
// ─────────────────────────────────────────────────────────────────────────────
// ADMIN (DEPARTMENT HEAD) CHAT HISTORY SCREEN
// Direct adaptation of worker_chat_history_screen.dart -- dedicated,
// read-only view of the signed-in Head's full chat history. Backend scopes
// /api/chat/history/ to request.user, so this can never show another
// user's messages, same as the worker version.
//
// Pushed on top of DepartmentHeadChatScreen (Navigator.push, not a sidebar
// destination), so it keeps the worker screen's AppBar+back-arrow shape
// rather than the DepartmentHeadSidebar/DepartmentHeadTopHeader shell used by top-level
// admin pages -- that shell has no back button or actions slot, and this
// is a sub-page, not a nav destination.
//
// Difference from the worker version: the bot avatar uses the shared
...
class DepartmentHeadChatHistoryScreen extends StatefulWidget {
  ...
  // When opened from the drawer's "Recent" list, [sessionId] pins this
  // screen to that one past chat instead of the Head's latest/active
  ...
  @override
  State<DepartmentHeadChatHistoryScreen> createState() => _DepartmentHeadChatHistoryScreenState();
}

class _DepartmentHeadChatHistoryScreenState extends State<DepartmentHeadChatHistoryScreen> {
...
class _HistoryBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const AiMascotAvatar(state: MascotState.idle, size: 30),
